...
SearchBtn.addEventListener("click", () => {
  const filteredData = mainData.filter(
    (elem) => elem.title.toUpperCase().includes(SearchProd.value.toUpperCase())
  );
...
az.addEventListener("click", () => {
  const filteredData = mainData.slice().sort((a, b) => a.title.localeCompare(b.title));
  createCart(filteredData);
});

za.addEventListener("click", () => {
  const filteredData = mainData.slice().sort((a, b) => b.title.localeCompare(a.title));
  createCart(filteredData);
});
